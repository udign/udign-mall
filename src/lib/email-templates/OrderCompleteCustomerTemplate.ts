...
export const OrderCompleteCustomerTemplate = (data: OrderCompleteEmailData): string => {
  const {
    siteName,
    siteUrl,
    orderId,
  ...
  return `
    ...
    <body>
      <div class="container">
        <div class="inner">
          <h1 class="header">${siteName} - 주문이 접수되었습니다.</h1>
          ...
          ${
            adminPhone
              ? `
          <div class="order-info">
            배송지를 변경하실 고객님은 <strong>${adminPhone}</strong>로 연락주시기 바랍니다.
          </div>
          `
              : ''
          }

          <a href="${siteUrl}/my-udign" target="_blank" class="cta-button">주문상세내역 확인</a>
        </div>
      </div>
    </body>
    </html>
